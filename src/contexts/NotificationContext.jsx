...
export const NotificationProvider = ({ children }) => {
  ...
  const fetchNotifications = useCallback(
    async ({ append = false, page = null, ...params } = {}) => {
      ...
      try {
        ...
        setError(null);
        const nextPage =
          page ?? (append ? (pagination?.current_page ?? 1) + 1 : 1);
        ...
        setPagination({
          ...
          per_page:
            payloadPagination.per_page ??
            payloadPagination.limit ??
            DEFAULT_PAGE_SIZE,
          total_items:
            payloadPagination.total_items ??
            payload.total_notifications ??
            fetchedNotifications.length,
          total_pages:
            payloadPagination.total_pages ??
            Math.max(
              1,
              Math.ceil(
                (payloadPagination.total_items ??
                  payload.total_notifications ??
                  fetchedNotifications.length) /
                  (payloadPagination.per_page ?? DEFAULT_PAGE_SIZE),
              ),
            ),
        });
      } catch (err) {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("versions:updated", handleVersionsUpdate);
