...
function Button({ children, className = '', isLoading, disabled, preventDoubleClick = true, onClick, ...props }) {
  ...
  return (
    <button
      className={`px-3 py-2 md:px-6 md:py-3 rounded-lg text-sm md:text-base font-semibold text-black ${isDisabled ? 'bg-blue-400 cursor-not-allowed' : 'bg-[#8DD8F5] hover:bg-[#1F2937] hover:text-white'} focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition duration-200 ${className}`}
  disabled={isDisabled}
  onClick={handleClick}
      {...props}
    >
