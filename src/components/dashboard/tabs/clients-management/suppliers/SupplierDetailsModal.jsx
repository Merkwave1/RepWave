...
const DetailItem = ({ icon, label, value, valueClassName = 'text-[#1F2937]' }) => (
  <div className="
  ...
  ">
    ...
    <div className="flex flex-col flex-1 min-w-0">
      ...
      <span className={`text-sm font-bold break-words leading-snug ${valueClassName}`}>
        {value ?? 'غير متوفر'}
      </span>
    </div>
  </div>
    ...
    <div className="relative overflow-hidden rounded-t-xl">
      ...
      <div className="relative p-6 flex items-start justify-between">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm text-[#8DD8F5]/80">
            تفاصيل وبيانات المورد
          </p>
        </div>
          ...
          {stats.map(s => (
            ...
            >
              <div className="flex items-center gap-2 mb-1 text-xs font-semibold text-[#1F2937]">
                {React.cloneElement(s.icon, { className: 'h-4 w-4 text-[#8DD8F5]' })}
                {s.label}
              </div>
