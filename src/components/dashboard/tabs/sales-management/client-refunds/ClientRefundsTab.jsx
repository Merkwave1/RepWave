        ...
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="w-40 px-4 py-3 text-center text-xs font-bold text-gray-700 uppercase">
                إجراءات
              </th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-100">
            {refunds.map((item, idx) => (
              <tr
                key={item.client_refunds_id || item.id || idx}
              ...
              >
                ...
                <td className="px-4 py-3 text-gray-900 font-medium border-r border-gray-200">
                  {item.client_name || item.clients_company_name || "-"}
                </td>
                <td className="px-4 py-3 text-gray-600 border-r border-gray-200">
                  {item.safe_name || "غير محدد"}
                </td>
                <td className="px-4 py-3 text-gray-600 border-r border-gray-200">
                  {item.payment_method_name || item.method_name || "-"}
                </td>
                <td className="px-4 py-3 text-gray-600 border-r border-gray-200">
                  {formatDate(item.client_refunds_date || item.refund_date)}
                </td>
                <td className="px-4 py-3 text-gray-600 border-r border-gray-200 font-semibold">
                  {formatCurrency(item.client_refunds_amount || item.amount)}
                </td>
                <td className="px-4 py-3 text-gray-600 text-sm border-r border-gray-200">
                  {item.client_refunds_notes || item.notes || "لا توجد ملاحظات"}
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center justify-center gap-2">
                    <button
                      title="عرض"
                      onClick={() => onView(item)}
                      className="p-2 rounded-md border border-gray-200 hover:bg-gray-50 text-gray-600 hover:text-gray-800"
                    >
                    ...
                    <button
                      title="تعديل"
                      onClick={() => onEdit(item)}
                      className="p-2 rounded-md border border-gray-200 hover:bg-gray-50 text-indigo-600 hover:text-indigo-800"
                    >
                    ...
                    <button
                      ...
                      className="p-2 rounded-md border border-gray-200 hover:bg-red-50 text-red-600 hover:text-red-800"
                    >
...
export default function ClientRefundsTab() {
  ...
  const loadRefunds = useCallback(async () => {
    ...
    try {
      ...
      const list =
        res.client_refunds || res.data?.client_refunds || res.data || res || [];
      setRefunds(Array.isArray(list) ? list : []);
    } catch (e) {
      setError(e.message || "Error loading client refunds");
      setGlobalMessage({
        type: "error",
        message: "فشل في تحميل مرتجعات العملاء.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const loadContext = useCallback(async () => {
    try {
      ...
      const { getPaymentMethods } =
        await import("../../../../../apis/payment_methods");
      ...
      if (methodsRes.status === "fulfilled") {
        setPaymentMethods(
          methodsRes.value.payment_methods || methodsRes.value || [],
        );
      }
  ...
  const filtered = useMemo(() => {
    ...
    if (term) {
      list = list.filter(
        (i) =>
          (i.client_name || i.clients_company_name || "")
            .toLowerCase()
            .includes(term) ||
          (i.client_refunds_notes || i.notes || "")
            .toLowerCase()
            .includes(term) ||
          (i.payment_method_name || i.method_name || "")
            .toLowerCase()
            .includes(term),
      );
    }
  ...
  const confirmDelete = async () => {
    try {
      await deleteClientRefund(
        refundToDelete.client_refunds_id || refundToDelete.id,
      );
    ...
    return (
      <>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0">
          <h3 className="text-2xl font-bold text-gray-800 flex-none">
            مرتجعات للعملاء (Refunds)
          </h3>
          <div className="relative flex-grow mx-4">
            <input
              ...
              placeholder="ابحث عن مرتجعات العملاء..."
              className="w-full pl-4 pr-10 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              dir="rtl"
            />
            {searchTerm && (
              <button
