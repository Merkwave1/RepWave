...
export default function ClientCashTab() {
  ...
  const loadContext = useCallback(async () => {
    try {
      ...
      setGlobalMessage?.({
        type: "error",
        message: "فشل تحميل بيانات الفلاتر.",
      });
    }
  }, [setGlobalMessage]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (appliedFilters.methodId)
        params.payment_method_id = appliedFilters.methodId;
      ...
      const overall_total =
        Number(totals.overall_total ?? payments_total + refunds_total) ||
        payments_total + refunds_total;
      const payments_amount_total =
        Number(totals.payments_amount_total ?? 0) || 0;
      const refunds_amount_total =
        Number(totals.refunds_amount_total ?? 0) || 0;
      setServerTotals({
        payments_total,
        refunds_total,
        overall_total,
      ...
      const nextTotalCount =
        Number(
          pagination.total_count ?? overall_total ?? movementList.length,
        ) || 0;
      const nextTotalPages =
        Number(
          pagination.total_pages ??
            (overall_total > 0 && perPage
              ? Math.ceil(overall_total / perPage)
              : 1),
        ) || 1;
      ...
      setGlobalMessage?.({
        type: "error",
        message: "فشل في تحميل حركات العملاء.",
      });
      setMovements([]);
      setServerTotals({
        payments_total: 0,
    ...
    return (movements || [])
      .map((row, index) => {
        const type =
          row.movement_type ||
          row.type ||
          (row.client_payments_id ? "payment" : "refund");
        const isPayment = type === "payment";
        const id = isPayment
          ? (row.client_payments_id ??
            row.movement_id ??
            row.id ??
            row.client_refunds_id)
          : (row.client_refunds_id ??
            row.movement_id ??
            row.id ??
            row.client_payments_id);
        ...
        const rawDate =
          ...
          row.date;
        const tsValue =
          row.sort_datetime ?? rawDate ?? row.created_at ?? row.updated_at;
        ...
        const clientName =
          row.client_name ?? row.clients_company_name ?? row.client ?? "-";
        const safeName = row.safe_name ?? row.safes_name ?? "";
        const methodName =
          row.method_name ?? row.payment_method_name ?? row.method ?? "";
  ...
  const totalItems = useMemo(() => {
    if (typeFilter === "payment")
      return Number(serverTotals.payments_total) || 0;
  ...
  const activeFilterChips = useMemo(() => {
    ...
    if (appliedFilters.type !== "all")
      chips.push({
        key: "type",
        label: "النوع",
        value: typeOptions.find((opt) => opt.value === appliedFilters.type)
          ?.label,
        tone: "purple",
    ...
    if (appliedFilters.clientId)
      chips.push({
        ...
        value: clientOptions.find(
          (opt) => opt.value === appliedFilters.clientId,
        )?.label,
        tone: "yellow",
        onRemove: () => setSelectedClient(""),
      });
    if (appliedFilters.safeId)
      chips.push({
        key: "safe",
        label: "الخزنة",
        value: safeOptions.find((opt) => opt.value === appliedFilters.safeId)
          ?.label,
        tone: "indigo",
        onRemove: () => setSelectedSafe(""),
      });
    if (appliedFilters.methodId)
      chips.push({
        key: "method",
        ...
        value: methodOptions.find(
          (opt) => opt.value === appliedFilters.methodId,
        )?.label,
        tone: "teal",
  ...
  const handlePrint = useCallback(
    async (row) => {
      try {
        ...
        const type =
          dataRow.movement_type ||
          dataRow.type ||
          (dataRow.client_payments_id ? "payment" : "refund");
        ...
        if (type === "payment") {
          ...
          try {
            const { getClientPaymentDetails } =
              await import("../../../../../apis/client_payments.js");
            const resp = await getClientPaymentDetails(
              dataRow.client_payments_id || dataRow.id || dataRow.payment_id,
            );
            details = resp?.payment_details || resp?.data || resp || null;
          } catch {
            console.warn(
              "Failed to fetch payment details, falling back to row data",
            );
            details = dataRow;
          }
        } else {
          ...
          try {
            const mod =
              await import("../../../../../apis/client_refunds.js").catch(
                () => null,
              );
            if (mod && typeof mod.getClientRefundDetails === "function") {
              const resp = await mod.getClientRefundDetails(
                dataRow.client_refunds_id || dataRow.id,
              );
        ...
        const amount =
          details.client_payments_amount ??
          details.movement_amount ??
          details.client_refunds_amount ??
          details.amount ??
          0;
        const dateStr =
          details.client_payments_date ??
          details.movement_date ??
          details.client_refunds_date ??
          details.date ??
          details.created_at ??
          "";
  ...
  const columnsMemo = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (item) => (
          <span
            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${item.type === "payment" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
          >
