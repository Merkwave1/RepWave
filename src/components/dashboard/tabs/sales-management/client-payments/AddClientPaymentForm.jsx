// src/components/dashboard/tabs/sales-management/client-payments/AddClientPaymentForm.jsx
import React, { useMemo, useState } from "react";
import { XMarkIcon, CurrencyDollarIcon } from "@heroicons/react/24/outline";
import Loader from "../../../../common/Loader/Loader";
import SearchableSelect from "../../../../common/SearchableSelect/SearchableSelect";
import { addClientPayment } from "../../../../../apis/client_payments";
import NumberInput from "../../../../common/NumberInput/NumberInput";
import useCurrency from "../../../../../hooks/useCurrency";

const AddClientPaymentForm = ({
  onClose,
  onSubmit,
  safes = [],
  clients = [],
  ...
  const handleSubmit = async (e) => {
    ...
    const selectedClient = (clients || []).find(
      (c) => String(c.clients_id || c.id) === String(formData.client_id),
    );
    const clientName =
      selectedClient?.clients_company_name ||
      selectedClient?.company_name ||
      selectedClient?.name ||
      "هذا العميل";
    const amountNumber = parseFloat(formData.amount || 0) || 0;
    const amountDisplay = `${amountNumber.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${symbol}`;
    const confirmSave = window.confirm(
      `هل تريد حفظ دفعة بقيمة ${amountDisplay} للعميل ${clientName}؟`,
    );
    ...
    try {
      await addClientPayment({
        ...
        payment_date: nowDateTimeForApi(),
      ...
      setErrors({ submit: err.message || "حدث خطأ أثناء إضافة الدفعة" });
    } finally {
