...
export default function SalesOrderDetailsModal({ order, onClose }) {
  ...
  const itemsArray = Array.isArray(currentOrder.items)
    ? currentOrder.items
    : [];
      ...
      const taxRate =
        parseFloat(
          it.sales_order_items_tax_rate ||
            it.sales_order_items_tax_percent ||
            it.sales_order_items_tax ||
            0,
        ) || 0;
      const explicitTax =
        parseFloat(it.sales_order_items_tax_amount || it.tax_amount || 0) || 0;
    ...
    try {
      ...
      const itemsForPrint = items.map((item, idx) => {
        ...
        const remainingQuantity = Math.max(
          0,
          orderedQuantity - deliveredQuantity - returnedQuantity,
        );
        ...
        const taxRate =
          parseFloat(
            item.sales_order_items_tax_rate ||
              item.sales_order_items_tax_percent ||
              item.sales_order_items_tax ||
              0,
          ) || 0;
        ...
        const totalPrice = parseFloat(
          item.sales_order_items_total_price ||
            orderedQuantity * unitPrice - discount + taxAmount ||
            0,
        );
        return {
          serial: idx + 1,
              ...
              <section id="summary" className="space-y-4">
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
                  ...
                  <SummaryCard
                    label="العميل"
                    value={
                      currentOrder.clients_company_name ||
                  ...
                  <SummaryCard
                    label="المستودع"
                    value={currentOrder.warehouse_name || "غير محدد"}
                  />
                  <SummaryCard
                    label="التاريخ"
                    value={
                      currentOrder.sales_orders_order_date
                        ? new Date(
                            currentOrder.sales_orders_order_date,
                  ...
                  <SummaryCard
                    label="عدد البنود"
                    value={(currentOrder.items?.length || 0).toLocaleString(
                      "en-US",
                    )}
                  />
                  <SummaryCard
                    ...
                    value={getStatusDisplay(
                      currentOrder.sales_orders_status || currentOrder.status,
                    )}
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
                  ...
                  <SummaryCard
                    label="خصم المنتجات"
                    value={formatCurrency(itemsDiscountTotal)}
                    compact
                  />
                  <SummaryCard
                    label="خصم الطلب"
                    value={formatCurrency(orderLevelDiscount)}
                    compact
                  />
                  <SummaryCard
                    label="إجمالي الخصم"
                    value={formatCurrency(combinedDiscount)}
                    compact
                  />
                  <SummaryCard
                    label="الضريبة"
                    value={formatCurrency(totalTaxValue)}
                    compact
                  />
                  <SummaryCard
                    label="الإجمالي"
                    value={formatCurrency(
                      parseFloat(
                        currentOrder.sales_orders_total_amount ||
                ...
                {/* Anchor Nav */}
                <nav
                  className="no-print flex flex-wrap gap-2 text-xs font-medium mt-2"
                  aria-label="أقسام التفاصيل"
                >
                  <a
                    href="#items"
                    className="px-3 py-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-700 transition"
                  >
                    المنتجات
                  </a>
                  <a
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="text-lg font-semibold text-gray-900">
                      ...
                      <span className="text-sm font-normal text-gray-500">
                        ({currentOrder.items.length} منتج)
                      </span>
                    </h4>
                    <span className="hidden sm:inline text-xs text-gray-500">
                      تم التحديث:{" "}
                      {currentOrder.sales_orders_updated_at
                        ? new Date(
                            currentOrder.sales_orders_updated_at,
                          ).toLocaleDateString("en-GB")
                        : "—"}
                          ...
                          return (
                            <tr key={index} className="hover:bg-indigo-50/40">
                              ...
                              <td className="px-3 py-2 text-gray-900">
                                {formatCurrency(
                                  parseFloat(
                                    item.sales_order_items_unit_price || 0,
                                  ),
                                )}
                              </td>
                              <td className="px-3 py-2 text-gray-500">
                                {formatCurrency(
                                  parseFloat(
                                    item.sales_order_items_discount_amount ||
                              ...
                              <td className="px-3 py-2 text-gray-500">
                                {(() => {
                                  ...
                                  const hasTax =
                                    item.sales_order_items_has_tax ||
                                    item.sales_order_items_has_tax === 1 ||
                                    taxRate > 0;
                                  let taxAmount =
                                    parseFloat(
                                      item.sales_order_items_tax_amount ||
                                        item.tax_amount ||
                                        0,
                                    ) || 0;
                                  ...
                                  return hasTax
                                    ? `${taxRate}% (${formatCurrency(taxAmount)})`
                                    : "لا يوجد";
                                })()}
                              </td>
                              <td className="px-3 py-2 font-semibold text-gray-900">
                                {formatCurrency(
                                  parseFloat(
                                    item.sales_order_items_total_price || 0,
                                  ),
                                )}
                              </td>
                            </tr>
              ...
              >
                ...
                <div className="mt-6 grid gap-4 sm:grid-cols-2">
                  <InfoStat
                    label="تاريخ الإنشاء"
                    value={
                      currentOrder.sales_orders_created_at
                        ? new Date(
                            currentOrder.sales_orders_created_at,
  ...
  if (badgeColor) {
    return (
      <div className={containerCls}>
        ...
        <span
          className={`inline-flex w-fit px-2 py-1 rounded-full text-xs font-semibold mt-1 ${badgeColor}`}
        >
          {value}
        </span>
      </div>
...
function InfoStat({ label, value, highlight }) {
  return (
    <div
      className={`rounded-md border bg-white p-3 flex flex-col gap-1 ${highlight ? "ring-1 ring-green-200" : ""}`}
    >
      <span className="text-[11px] text-gray-500 font-medium">{label}</span>
      <span
