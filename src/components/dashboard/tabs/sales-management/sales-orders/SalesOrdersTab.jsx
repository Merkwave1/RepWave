...
const getOrderTotalAmount = (order) => {
  ...
  const directTotal = parseAmount(
    order?.sales_orders_total_amount ??
      order?.total_amount ??
      order?.grand_total,
  );
...
const calculateInvoiceBalanceDelta = (
  previousStatus,
  nextStatus,
  previousTotalAmount,
  nextTotalAmount,
) => {
  const prevTotal = Number.isFinite(previousTotalAmount)
    ? previousTotalAmount
    : 0;
...
export default function SalesOrdersTab({
  ...
  title = "أوامر البيع",
  subtitle = "إدارة وتتبع أوامر البيع",
  ...
  const [selectedDeliveryStatusFilter, setSelectedDeliveryStatusFilter] =
    useState("");
  const [selectedClientFilter, setSelectedClientFilter] = useState("");
  const [selectedRepresentativeFilter, setSelectedRepresentativeFilter] =
    useState("");
  ...
  const loadRepresentativesFromCache = useCallback(async () => {
    try {
      const raw =
        ...
        localStorage.getItem("appRepresentatives") ||
    ...
    try {
      ...
      if (Array.isArray(ordersResp)) {
        ...
        const totalRaw = Number(
          ordersResp.total ?? ordersResp.count ?? ordersResp.total_items ?? NaN,
        );
        const perRaw = Number(
          ordersResp.per_page ?? ordersResp.limit ?? itemsPerPage,
        );
        const pageRaw = Number(
          ordersResp.page ?? ordersResp.current_page ?? currentPage,
        );
        const pagesRemaining = Number.isFinite(
          Number(ordersResp.pages_remaining),
        )
          ? Number(ordersResp.pages_remaining)
          : null;
        const totalPagesRaw = Number(ordersResp.total_pages ?? NaN);
        const computedTotalPages =
          Number.isFinite(totalRaw) && perRaw
            ? Math.max(1, Math.ceil(totalRaw / perRaw))
            : pagesRemaining != null
              ? pagesRemaining + pageRaw
              : undefined;
        const normalized = {
          total: Number.isFinite(totalRaw)
            ? totalRaw
            : Number.isFinite(incomingPagination?.total_items)
              ? Number(incomingPagination.total_items)
              : undefined,
          per_page: Number.isFinite(perRaw)
            ? perRaw
            : Number(
                incomingPagination?.per_page ??
                  incomingPagination?.limit ??
                  itemsPerPage,
              ),
          page: Number.isFinite(pageRaw)
            ? pageRaw
            : Number(
                incomingPagination?.page ??
                  incomingPagination?.current_page ??
                  currentPage,
              ),
          total_pages: Number.isFinite(totalPagesRaw)
            ? totalPagesRaw
            : (incomingPagination?.total_pages ?? computedTotalPages),
          pages_remaining:
            pagesRemaining ?? incomingPagination?.pages_remaining,
        };
              ...
              total_pages: Number(
                p.total_pages ??
                  Math.max(
                    1,
                    Math.ceil(
                      Number(p.total ?? p.total_items ?? 0) /
                        Number(p.per_page ?? p.limit ?? itemsPerPage),
                    ),
                  ),
              ),
            }
      ...
      const extractedOrders = rawOrders.map((order) => ({
        ...
        sales_orders_representative_id:
          order.sales_orders_representative_id ||
          order.representative_id ||
          order.representative_user_id,
  ...
  const activeChips = useMemo(() => {
    ...
    if (appliedFilters.dateFrom || appliedFilters.dateTo) {
      chips.push({
        key: "date",
        label: "التاريخ",
        value: `${appliedFilters.dateFrom || "من البداية"} - ${appliedFilters.dateTo || "حتى النهاية"}`,
        tone: "gray",
        onRemove: () => {
          setDateFrom("");
          setDateTo("");
    ...
    if (appliedFilters.clientId) {
      const label =
        clients.find((c) => String(c.clients_id) === appliedFilters.clientId)
          ?.clients_company_name || appliedFilters.clientId;
      chips.push({
        key: "client",
        label: "العميل",
        value: label,
    ...
    if (appliedFilters.representativeId) {
      const label =
        representatives.find(
          (r) => String(r.users_id) === appliedFilters.representativeId,
        )?.users_name || appliedFilters.representativeId;
      chips.push({
        key: "rep",
        label: "المندوب",
        value: label,
    ...
    if (appliedFilters.warehouseId) {
      const label =
        warehouses.find(
          (w) =>
            String(w.warehouses_id || w.warehouse_id) ===
            appliedFilters.warehouseId,
        )?.warehouses_name || appliedFilters.warehouseId;
      chips.push({
        key: "warehouse",
        label: "المستودع",
        value: label,
    ...
    if (appliedFilters.status) {
      const label =
        statusOptions.find((s) => s.value === appliedFilters.status)?.label ||
    ...
    if (appliedFilters.deliveryStatus) {
      const label =
        deliveryStatusOptions.find(
          (s) => s.value === appliedFilters.deliveryStatus,
        )?.label || appliedFilters.deliveryStatus;
      chips.push({
        key: "delivery",
        label: "حالة التسليم",
        value: label,
  ...
  const handleAddOrder = async (orderData) => {
    ...
    const delta = calculateInvoiceBalanceDelta(
      null,
      orderData?.sales_orders_status,
      0,
      orderTotal,
    );
    ...
    try {
      ...
      setGlobalMessage({
        type: "success",
        message: "تم إضافة أمر البيع بنجاح!",
      });
    } catch (error) {
      console.error("Error adding sales order:", error);
      setGlobalMessage({
        type: "error",
        message: `فشل في إضافة أمر البيع: ${error.message}`,
      });
    }
  };
  ...
  const handleUpdateOrder = async (orderData) => {
    ...
    const previousStatus =
      previousOrder?.sales_orders_status ?? previousOrder?.status ?? null;
    const previousTotal = getOrderTotalAmount(previousOrder);
    const nextStatus =
      orderData?.sales_orders_status ?? orderData?.status ?? null;
    const nextTotal = getOrderTotalAmount(orderData);
    const clientId =
      orderData?.sales_orders_client_id ??
      previousOrder?.sales_orders_client_id ??
      previousOrder?.client_id ??
      null;
    const delta = calculateInvoiceBalanceDelta(
      previousStatus,
      nextStatus,
    ...
    try {
      ...
      setGlobalMessage({
        type: "error",
        message: `فشل في تحديث أمر البيع: ${error.message}`,
      });
    }
  };
  ...
  const clientOptions = useMemo(
    () =>
      (clients || []).map((c) => ({
        value: String(c.clients_id || c.id),
        label:
  ...
  const warehouseOptions = useMemo(
    () =>
      (warehouses || []).map((w) => ({
        value: String(w.warehouses_id || w.warehouse_id || w.id),
        label:
          w.warehouses_name ||
          w.warehouse_name ||
          w.name ||
          `#${w.warehouses_id || w.warehouse_id || w.id}`,
      })),
    [warehouses],
  );
  ...
  const totalOrders = useMemo(() => {
    ...
    if (pagination?.total_pages && (pagination?.per_page || itemsPerPage)) {
      const estimate =
        ...
        Number(pagination.per_page || itemsPerPage);
  ...
  const handleViewOrderDetails = async (order) => {
    try {
      ...
      const detailedOrder = await getSalesOrderDetails(
        order.sales_orders_id || order.id,
      );
      ...
      setGlobalMessage({
        type: "error",
        message: `فشل في تحميل تفاصيل الأمر: ${error.message}`,
      });
    }
  };
  ...
  const handleDeliver = async (order) => {
    try {
      setGlobalMessage({
        type: "info",
        message: "جاري تحميل بيانات الأمر للتسليم...",
      });
      const detailedOrder = await getSalesOrderDetails(
        order.sales_orders_id || order.id,
      );
      ...
      setGlobalMessage({
        type: "error",
        message: `فشل في تحميل بيانات الأمر: ${error.message}`,
      });
    }
  };
  ...
  const handlePrint = async (order) => {
    try {
      ...
      const statusValue = (
        orderDetails.sales_orders_status ||
      ...
      const orderInfoHeading = isQuotePrint
        ? "معلومات العرض"
        : "معلومات الفاتورة";
      ...
      const itemsForPrint =
        orderDetails.items?.map((item, index) => ({
          ...
          tax_rate: item.sales_order_items_has_tax
            ? parseFloat(item.sales_order_items_tax_rate || 0)
            : 0,
          ...
          remaining:
            parseFloat(item.sales_order_items_quantity || 0) -
      ...
      const itemsDiscountTotal = itemsForPrint.reduce(
      ...
      const orderLevelDiscount =
        parseFloat(
          orderDetails.sales_orders_discount_amount ||
            order.sales_orders_discount_amount ||
            0,
        ) || 0; // خصم الطلب
      const combinedDiscount = itemsDiscountTotal + orderLevelDiscount; // إجمالي الخصم
      const totalTax = itemsForPrint.reduce(
        (sum, item) => sum + item.tax_amount,
        0,
      );
      const grandTotal = parseFloat(
        orderDetails.sales_orders_total_amount ||
          order.sales_orders_total_amount ||
          subtotal - combinedDiscount + totalTax ||
          0,
      );
  ...
  return (
    <div className="p-6">
      {/* ── Always-visible list view ── */}
      <CustomPageHeader
        title={title}
        subtitle={subtitle}
        icon={<TruckIcon className="h-6 w-6 text-[#1F2937]" />}
      ...
      <FilterBar
        ...
        selectFilters={[
          ...
          },
          {
            key: "representative",
            label: "المندوب",
            value: selectedRepresentativeFilter,
            onChange: (v) => setSelectedRepresentativeFilter(v),
            ...
            : null,
          {
            ...
            onChange: (v) => setSelectedDeliveryStatusFilter(v),
      ...
      <SalesOrderListView
        ...
        onEdit={async (order) => {
          try {
            setGlobalMessage({
              type: "info",
              message: "جاري تحميل بيانات الأمر للتعديل...",
            });
            const detailedOrder = await getSalesOrderDetails(
              order.sales_orders_id || order.id,
            );
