  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <div
        ...
        className="w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm cursor-pointer bg-white hover:border-gray-400 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500"
      >
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                ...
                className="flex-1 px-3 py-2 bg-gray-200 text-gray-700 text-sm rounded-lg hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500"
              >
                مسح
              </button>
            </div>
...
const unwrapReturnData = (resp) => {
  if (
    resp &&
    resp.data &&
    (resp.data.returns_id || Array.isArray(resp.data.items))
  )
    return resp.data;
  return resp;
};
...
export default function SalesReturnsTab() {
  ...
  const handleViewDetails = React.useCallback(
    async (returnItem) => {
      ...
      try {
        const resp = await getSalesReturnDetails(
          returnItem.returns_id || returnItem.id,
        );
  ...
  const handlePrintReturnAction = React.useCallback(
    async (returnItem) => {
      try {
        const resp = await getSalesReturnDetails(
          returnItem.returns_id || returnItem.id,
  ...
  const loadAllSalesReturnData = useCallback(
      ...
      try {
        const filterParams = {
          ...
          ...(appliedFilters.searchTerm && {
            search: appliedFilters.searchTerm.trim(),
          }),
          ...
          ...(appliedFilters.clientId && {
            client_id: appliedFilters.clientId,
          }),
          ...(appliedFilters.representativeId && {
            representative_id: appliedFilters.representativeId,
          }),
          ...(appliedFilters.warehouseId && {
            warehouse_id: appliedFilters.warehouseId,
          }),
        };
        ...
        if (forceApiRefresh) {
          setGlobalMessage({
            type: "error",
            message: `فشل في تحديث مرتجعات البيع: ${error.message}`,
          });
        }
      } finally {
        setLoading(false);
  ...
  const handleUpdateReturn = async (id, returnData) => {
    try {
      ...
      setGlobalMessage({
        type: "error",
        message: `خطأ في تحديث مرتجع البيع: ${error.message}`,
      });
    }
  };
  ...
  const filteredReturns = useMemo(() => {
    ...
    return salesReturns.filter((returnItem) => {
      ...
      const matchesSearch =
        ...
        String(
          returnItem.returns_return_number ?? returnItem.return_number ?? "",
        )
          .toLowerCase()
      ...
      const matchesStatus =
        !selectedStatusFilter ||
        (returnItem.returns_status || returnItem.status) ===
          selectedStatusFilter;
      const matchesClient =
        !selectedClientFilter ||
        (returnItem.returns_client_id || returnItem.client_id) ==
          selectedClientFilter;
    ...
    () => [
      {
        ...
        align: "center",
        sortAccessor: (it) =>
          Number(it.returns_id || it.id || it.sales_returns_id || 0),
        className: "w-24",
        render: (it) => {
        ...
        : []),
      {
        ...
        render: (it) => (
          <div className="font-medium text-gray-900">
            {it.clients_company_name ||
              it.client_name ||
              it.returns_client_name ||
              "غير محدد"}
          </div>
        ),
      },
      ...
      },
      {
        ...
        render: (it) => {
          ...
          return (
            <span
              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${toneClass}`}
            >
              {status}
            </span>
          );
      ...
      },
      {
        ...
        sortable: true,
        sortAccessor: (it) =>
          Number(it.returns_total_amount || it.total_amount || 0),
      ...
      },
      {
        ...
        render: (it) => (
          <div className="flex items-center justify-center gap-2">
            ...
            {it.returns_status !== "Processed" &&
              it.returns_status !== "Cancelled" && (
                <button
      ...
      <>
          ...
          activeChips={(() => {
            ...
            if (appliedFilters.status)
              chips.push({
                ...
                label: "الحالة",
                value:
                  statusOptions.find((s) => s.value === appliedFilters.status)
            ...
            if (appliedFilters.clientId)
              chips.push({
                ...
                label: "العميل",
                value:
                  clientOptions.find((c) => c.value === appliedFilters.clientId)
        ...
        <div className="mt-4">
          <GlobalTable
            data={sortedReturns}
            rowKey={(item) =>
              item.returns_id || item.id || item.sales_returns_id
            }
            ...
            <div className="w-full max-w-4xl my-4">
              {isSupportingDataLoaded ? (
                ...
                <div className="bg-white rounded-2xl p-10 text-center text-gray-500">
                  <div className="w-8 h-8 border-4 border-gray-200 border-t-rose-500 rounded-full animate-spin mx-auto mb-3" />
            ...
            <div className="w-full max-w-4xl my-4">
              {isSupportingDataLoaded && selectedReturn ? (
                <SalesReturnForm
                  ...
                  onSubmit={(returnData) =>
                    handleUpdateReturn(
                      selectedReturn.sales_returns_id ||
                        selectedReturn.returns_id ||
                        selectedReturn.id,
                      returnData,
                    )
                  }
  ...
  const printSalesReturn = async (data) => {
    ...
    const lineSummaries = items.map((it) => {
      const lineRaw =
        Number(it.return_items_total_price || it.total_price || 0) || 0;
      const providedTax =
        Number(it.return_items_tax_amount || it.tax_amount || 0) || 0;
      const qty = Number(it.return_items_quantity || it.quantity || 0) || 0;
      const orderedQty =
        Number(
          it.ordered_quantity ||
            it.sales_order_items_quantity ||
            it.sales_order_items_quantity ||
            0,
        ) || 0;
      ...
      const taxRate =
        Number(
          it.return_items_tax_rate ||
            it.tax_rate ||
            it._tax_rate ||
            it.sales_order_items_tax_rate ||
            0,
        ) || 0;
      const hasTax =
        Boolean(
          it.return_items_has_tax ?? it.has_tax ?? it.sales_order_items_has_tax,
        ) || taxRate > 0;
      return { it, lineRaw, providedTax, qty, orderedQty, taxRate, hasTax };
    });

    const rawSum = lineSummaries.reduce((sum, line) => sum + line.lineRaw, 0);
    const providedTaxSum = lineSummaries.reduce(
      (sum, line) => sum + line.providedTax,
      0,
    );

    const headerTotal =
      Number(data.returns_total_amount || data.total_amount || 0) || 0;
    const netPlusTax = rawSum + providedTaxSum;
    let rawIsNet = true;
    if (headerTotal > 0) {
      if (Math.abs(headerTotal - netPlusTax) < EPS) rawIsNet = true;
      else if (Math.abs(headerTotal - rawSum) < EPS) rawIsNet = false;
    }

    // Compute per-item discount (prorated for returned qty) and build rows including discount column
    let computedTaxSum = 0;

    const rows = lineSummaries
      .map(
        (
          { it, lineRaw, providedTax, qty, orderedQty, taxRate, hasTax },
          idx,
        ) => {
          ...
          const itemDiscountRaw =
            Number(
              it.return_items_discount_amount ||
                it.return_items_discount ||
                it.sales_order_items_discount_amount ||
                it.discount_amount ||
                0,
            ) || 0;
          const itemDiscountForReturned =
            (itemDiscountRaw / Math.max(1, orderedQty || 1)) * qty;
    ...
    const proratedOrderDiscount =
      headerOrderDiscount > 0 && totalOrderedQty > 0
        ? headerOrderDiscount * (totalReturnedQty / totalOrderedQty)
        : 0;
