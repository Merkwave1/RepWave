...
// APIs (mock)
import { mockProductsApi } from "../../../../mock/mockApiWrapper";
...
function ProductsTab() {
  ...
  const loadSupportData = useCallback(async () => {
    ...
    try {
      ...
      const productsResponse = await mockProductsApi.getAllProducts();
  ...
  const handleAddProduct = async (data) => {
    ...
    try {
      await mockProductsApi.addProduct(data);
      ...
      const colMatch =
        msg.match(/Column '\s*([^']+?)\s*' cannot be null/i) ||
        msg.match(/Column\s+`([^`]+)`\s+cannot be null/i);
      if (colMatch && colMatch[1]) {
        ...
        setAddFormError({
          field,
          message: `${friendly} مطلوب ولا يمكن أن يكون فارغاً.`,
        });
      } else {
        ...
        try {
          ...
          if (
            ...
            (parsed.errors || parsed.validation)
          ) {
            const errorsObj = parsed.errors || parsed.validation;
            const firstKey = Object.keys(errorsObj)[0];
            setAddFormError({
              field: firstKey,
              message: errorsObj[firstKey] || msg,
            });
          }
        } catch {
  ...
  const handleUpdateProduct = async (productId, data) => {
    setLoading(true);
    try {
      await mockProductsApi.updateProduct(productId, data);
      ...
      setGlobalMessage({
        type: "error",
        message: e.message || "فشل في تحديث المنتج.",
      });
      setLoading(false);
    }
  };
  ...
  const handleDeleteProduct = async () => {
    ...
    setLoading(true);
    try {
      await mockProductsApi.deleteProduct(selectedProduct.products_id);
      ...
      setGlobalMessage({
        type: "error",
        message: e.message || "فشل في حذف المنتج.",
      });
      setLoading(false);
    }
  };
  ...
  const uniqueBrands = useMemo(() => {
    const brands = new Set(
      products.map((p) => p.products_brand).filter(Boolean),
    );
  ...
  const totalVariants = useMemo(() => {
    return products.reduce(
      (acc, p) => acc + (Array.isArray(p.variants) ? p.variants.length : 0),
      0,
    );
  }, [products]);
  ...
  const uniqueSuppliersForFilter = useMemo(() => {
    ...
    const productSupplierIds = new Set(
      products.map((p) => p.products_supplier_id).filter(Boolean),
    );
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (term) {
      const getCategoryName = (id) =>
        categories.find((c) => c.categories_id == id)?.categories_name || "";
      ...
      currentFiltered = currentFiltered.filter(
        (p) =>
          ...
          getCategoryName(p.products_category_id)
            ?.toLowerCase()
            .includes(term) ||
          getSupplierName(p.products_supplier_id)
            ?.toLowerCase()
            .includes(term) ||
          p.variants?.some(
            (v) =>
              v.variant_name?.toLowerCase().includes(term) ||
              v.variant_sku?.toLowerCase().includes(term),
          ),
      );
    }
    if (selectedCategoryFilter)
      currentFiltered = currentFiltered.filter(
        (p) => p.products_category_id == selectedCategoryFilter,
      );
    if (selectedStatusFilter !== "")
      currentFiltered = currentFiltered.filter(
        (p) => p.products_is_active == selectedStatusFilter,
      );
    if (selectedBrandFilter)
      currentFiltered = currentFiltered.filter(
        (p) => p.products_brand === selectedBrandFilter,
      );
    if (selectedSupplierFilter)
      currentFiltered = currentFiltered.filter(
        (p) => p.products_supplier_id == selectedSupplierFilter,
      );
  ...
  const selectFilters = [
    ...
    {
      key: "supplier",
      options: (uniqueSuppliersForFilter || []).map((s) => ({
        value: s.id,
        label: s.name,
      })),
  ...
  if (selectedCategoryFilter)
    activeChips.push({
      ...
      value:
        categories.find(
          (c) => String(c.categories_id) === String(selectedCategoryFilter),
        )?.categories_name || selectedCategoryFilter,
      tone: "blue",
      onRemove: () => setSelectedCategoryFilter(""),
    });
  if (selectedStatusFilter !== "")
    activeChips.push({
      key: "status",
      label: "الحالة",
      value: selectedStatusFilter === "1" ? "نشط" : "غير نشط",
      tone: "green",
  ...
  if (selectedBrandFilter)
    activeChips.push({
      ...
      value: selectedBrandFilter,
      tone: "purple",
      onRemove: () => setSelectedBrandFilter(""),
    });
  if (selectedSupplierFilter)
    activeChips.push({
      key: "supplier",
      label: "المورد",
      value:
        uniqueSuppliersForFilter.find(
          (s) => String(s.id) === String(selectedSupplierFilter),
        )?.name || selectedSupplierFilter,
      tone: "teal",
      onRemove: () => setSelectedSupplierFilter(""),
    });

  return (
    <div className="p-2 md:p-4 relative" dir="rtl">
      {/* Always-visible list */}
      <CustomPageHeader
        title="إدارة المنتجات"
        subtitle="قائمة المنتجات وإدارتها"
        icon={<ShoppingBagIcon className="h-6 w-6 text-[#1F2937]" />}
        ...
        actionButton={
          <button
            ...
            className="px-3 py-2 bg-[#1F2937] text-[#8DD8F5] hover:bg-[#374151] hover:scale-110 rounded-md font-semibold"
          >
            إضافة منتج
          </button>
        }
      ...
      {!loading && !error && (
        <ProductsListView
          ...
          onDelete={(p) => {
            setSelectedProduct(p);
            setCurrentView("deleteConfirm-product");
          }}
          onViewDetails={(p) => {
            setSelectedProduct(p);
      ...
      {isOverlay && (
        <div
          ...
          onClick={() => setCurrentView("list-products")}
        />
      )}
      ...
      {currentView === "add-product" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
          <div
            className="w-full max-w-4xl my-auto relative"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setCurrentView("list-products")}
              className="absolute top-2 left-2 z-10 p-1.5 rounded-full bg-gray-100 hover:bg-red-100 hover:text-red-600 text-gray-500 transition-colors"
              title="إغلاق"
            >
      ...
      {currentView === "deleteConfirm-product" && (
