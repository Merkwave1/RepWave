...
function CalendarTab() {
  ...
  const getFilteredPlans = () => {
    return visitPlans.filter((plan) => {
      // Status filter
      if (showOnlyActivePlans && plan.visit_plan_status !== "Active")
        return false;
      if (selectedStatus && plan.visit_plan_status !== selectedStatus)
        return false;
  ...
  return (
    <div className="space-y-4 sm:space-y-6" dir="rtl">
      <CustomPageHeader
        ...
        icon={<CalendarDaysIcon className="w-7 h-7 text-[#1F2937]" />}
        ...
        actionButton={[
          <button
            ...
            type="button"
            onClick={goToToday}
            className="bg-[#1F2937] self-end hover:scale-105 text-[#8DD8F5] hover:bg-[#374151] px-4 py-2 rounded-lg flex items-center gap-2 transition-all duration-200 shadow-md hover:shadow-lg font-bold text-lg"
          >
            اليوم
          </button>,
