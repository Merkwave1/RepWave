...
export default function UpdatePurchaseReturnFormEnhanced({
  ...
  useEffect(() => {
    if (formData.supplier_id) {
      const filteredOrders = purchaseOrders.filter(
        (order) =>
          order.purchase_orders_supplier_id?.toString() ===
            formData.supplier_id &&
          ["Ordered", "Shipped", "Received", "Partially Received"].includes(
        ...
        if (field === "purchase_order_item_id" && value) {
          ...
          if (returnableInfo) {
            Object.assign(updatedItem, {
              ...returnableInfo,
              // For update form, we show current available quantities as-is since we're editing an existing return
              available_to_return_received:
                returnableInfo.available_to_return_received,
              available_to_return_not_received:
                returnableInfo.available_to_return_not_received,
  ...
  const getOrderItemInfo = useCallback(
    (orderItemId) => {
      const orderItem = availableOrderItems.find(
        (item) => item.purchase_order_items_id?.toString() === orderItemId,
      );
      if (!orderItem) return null;

      const product = products.find(
        (p) =>
          p.product_variant_id === orderItem.purchase_order_items_variant_id,
      );
      const baseUnit = baseUnits.find(
        (u) => u.base_unit_id === product?.product_variant_base_unit_id,
      );
      const packagingType = packagingTypes.find(
        (pt) =>
          pt.packaging_type_id ===
          orderItem.purchase_order_items_packaging_type_id,
      );
      ...
      return {
        ...
        packagingType:
          packagingType?.packaging_type_name ||
        ...
        availableToReturnReceived:
          returnableInfo.available_to_return_received || 0,
        availableToReturnNotReceived:
          returnableInfo.available_to_return_not_received || 0,
        receiveStatus: returnableInfo.receive_status || "لم يتم الاستلام",
        totalAvailableToReturn:
          (returnableInfo.available_to_return_received || 0) +
          (returnableInfo.available_to_return_not_received || 0),
      };
    },
    ...
    formData.items.forEach((item, index) => {
      ...
      if (!item.quantity || quantity <= 0) {
        ...
        newErrors[`item_${index}_quantity`] =
          `الكمية تتجاوز المتاح للإرجاع (${item.total_available_to_return})`;
      }
    });
  ...
  const handleSubmit = async (e) => {
    ...
    const confirmMessage =
      `هل أنت متأكد من تحديث مرتجع الشراء؟\n\n` +
      `عدد الأصناف: ${itemCount}\n` +
      `المبلغ التقريبي: ${totalAmount.toFixed(2)}\n\n` +
      `سيتم تحديث المرتجع والمخزون.`;
                          ...
                          <div className="flex items-center gap-4 text-xs mt-1">
                            <div className="text-gray-500">
                              ...
                              {item.quantity_ordered ||
                                orderItemInfo?.orderedQuantity ||
                                "0"}
                            </div>
                            <div className="text-gray-500">
                              مستلم:{" "}
                              {item.quantity_received ||
                                orderItemInfo?.receivedQuantity ||
                                "0"}
                            </div>
                            ...
                            {item.available_to_return_not_received > 0 && (
                              <div className="text-orange-600 font-medium">
                                متاح للإرجاع (لم يتم استلامها):{" "}
                                {item.available_to_return_not_received}
                              </div>
                            )}
                            {item.available_to_return_received > 0 && (
                              <div className="text-blue-600 font-medium">
                                متاح للإرجاع (في المخزون):{" "}
                                {item.available_to_return_received}
                              </div>
                            )}
                            {/* Status badge */}
                            <div
                          ...
                          <div className="text-xs mt-1">
                            ...
                            item.available_to_return_received > 0 ? (
                              <span className="text-green-600">
                                متاح لارجاع{" "}
                                {item.available_to_return_not_received} لم يتم
                                استلامهم +{item.available_to_return_received} تم
                                استلامهم
                              </span>
                            ) : item.available_to_return_received > 0 ? (
                              <span className="text-blue-600">
                                متاح للارجاع:{" "}
                                {item.available_to_return_received} تم استلامهم
                              </span>
                            ) : item.available_to_return_not_received > 0 ? (
