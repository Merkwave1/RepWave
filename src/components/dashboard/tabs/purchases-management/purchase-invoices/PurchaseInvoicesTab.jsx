...
// English statuses (real backend) + Arabic equivalents (mock data)
...
export default function PurchaseInvoicesTab() {
      ...
      try {
        ...
        const itemsForPrint = transformedOrder.items.map((item, index) => {
          ...
          const discountAmount = parseFloat(
            item.purchase_order_items_discount_amount || 0,
          );
          const taxRate = parseFloat(item.purchase_order_items_tax_rate || 0);
          const hasTax =
            item.purchase_order_items_has_tax === 1 ||
            item.purchase_order_items_has_tax === "1";
  ...
  const loadData = useCallback(
    async (forceApiRefresh = false) => {
      ...
      setError(null);
      try {
        const statusCSV = selectedStatusFilter
          ? selectedStatusFilter
          : Array.from(INCLUDED_STATUSES).join(",");
        ...
        if (forceApiRefresh)
          setGlobalMessage({
            type: "success",
            message: "تم تحديث فواتير الشراء.",
          });
      } catch (e) {
  ...
  useEffect(() => {
    const rh = () => loadData(true);
    setChildRefreshHandler(rh);
  ...
  const uniqueSuppliersForFilter = useMemo(() => {
    const m = new Map();
    suppliers.forEach((s) => {
      if (s && (s.supplier_id || s.id))
        m.set(s.supplier_id || s.id, s.supplier_name || s.name);
    });
  ...
  const activeChips = useMemo(() => {
    ...
    if (selectedSupplierFilter) {
      const found = uniqueSuppliersForFilter.find(
        (opt) => String(opt[0] ?? opt.value) === String(selectedSupplierFilter),
      );
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (selectedStatusFilter) {
      cur = cur.filter(
        (o) => o.purchase_orders_status === selectedStatusFilter,
      );
    }
    if (selectedSupplierFilter) {
      cur = cur.filter(
        (o) => String(o.purchase_orders_supplier_id) === selectedSupplierFilter,
      );
    }
    cur.sort((a, b) => {
      const da = new Date(
  ...
  return (
    <div className="p-4" dir="rtl">
      <CustomPageHeader
        title="فواتير الشراء"
        icon={<DocumentTextIcon className="w-6 h-6 text-[#1F2937]" />}
      ...
      <FilterBar
        ...
        selectFilters={[
          ...
          },
          {
            ...
            options: uniqueSuppliersForFilter.map(([id, name]) => ({
              value: String(id),
              label: name,
            })),
          },
        ...
        (() => {
          const tp =
            itemsPerPage > 0
              ? Math.max(
                  1,
                  Math.ceil((pagination?.total_items || 0) / itemsPerPage),
                )
              : 1;
          return (
              ...
              <div className="text-sm md:text-base text-gray-700">
                ...
                {(pagination?.total_items ?? filteredOrders.length) > 0 && (
                  <>
                    ...
                    صفحة <span className="font-semibold">
                      {currentPage}
                    </span> من <span className="font-semibold">{tp}</span>
                  </>
                )}
              </div>
                ...
                <div className="flex items-center gap-2">
                  <div className="inline-flex overflow-hidden rounded-md border border-gray-300 bg-white shadow-sm">
                    <button
                      ...
                      className="px-2 py-1.5 text-sm hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                    ...
                    <button
                      ...
                      disabled={currentPage <= 1}
                      className="px-2 py-1.5 text-sm border-r border-gray-300 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      <ChevronRightIcon className="h-5 w-5" />
                    </button>
                    <span className="px-3 py-1.5 text-sm text-gray-700 bg-gray-50">
                      {currentPage}
                    ...
                    <button
                      ...
                      className="px-2 py-1.5 text-sm border-l border-gray-300 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed"
                    >
