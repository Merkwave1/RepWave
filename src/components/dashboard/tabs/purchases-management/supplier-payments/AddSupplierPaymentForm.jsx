...
const AddSupplierPaymentForm = ({ onClose, onSubmit, safes = [] }) => {
  ...
  const handleInputChange = (field, value) => {
    ...
    if (field === "supplier_id") {
      ...
      if (selectedSupplier) {
        ...
        const balance =
          selectedSupplier.supplier_balance || selectedSupplier.balance || 0;
  ...
  const handleSubmit = async (e) => {
    ...
    const supplierName =
      selectedSupplier?.supplier_name || selectedSupplier?.name || "هذا المورد";
    const amountDisplay = formatMoney(parseFloat(formData.amount || 0) || 0);

    const confirmSave = window.confirm(
      `هل تريد حفظ دفعة بقيمة ${amountDisplay} للمورد ${supplierName}؟`,
    );
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 backdrop-blur-sm bg-black/40 flex items-center justify-center z-50 p-2 sm:p-4">
        <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden mx-2 sm:mx-4">
                ...
                {formData.supplier_id && selectedSupplierBalance !== null && (
                  <div className="mt-2 p-2 bg-blue-50 rounded border border-blue-200">
                    <p className="text-xs text-gray-600">
                      رصيد المورد:{" "}
                      <span
                        className={`font-semibold ${selectedSupplierBalance > 0 ? "text-blue-600" : selectedSupplierBalance < 0 ? "text-red-600" : "text-gray-600"}`}
                      >
                ...
                {formData.safe_id && selectedSafeBalance !== null && (
                  <div className="mt-2 p-2 bg-green-50 rounded border border-green-200">
                    <p className="text-xs text-gray-600">
                      رصيد الخزنة:{" "}
                      <span
                        className={`font-semibold ${selectedSafeBalance > 0 ? "text-green-600" : selectedSafeBalance < 0 ? "text-red-600" : "text-gray-600"}`}
                      >
