...
export default function PurchaseOrdersTab() {
  ...
  const loadAllPurchaseOrderData = useCallback(
    async (forceApiRefresh = false, overrides = {}) => {
      ...
      try {
        ...
        const supplierToUse = supplierRaw
          ? Number.isNaN(Number(supplierRaw))
            ? supplierRaw
            : Number(supplierRaw)
          : undefined;
        const searchToUse =
        ...
        setGlobalMessage({
          type: "error",
          message: `فشل في تحميل بيانات أوامر الشراء: ${errorMessage}`,
        });
      } finally {
  ...
  const handleEdit = useCallback(
    async (order) => {
      try {
        ...
        setGlobalMessage({
          type: "error",
          message: `فشل في تحميل بيانات الطلب للتحرير: ${error.message}`,
        });
      } finally {
        setLoading(false);
      }
    ...
    async (order) => {
      try {
        ...
        const transformedOrder = {
          ...
          items:
            detailedOrder.items?.map((item) => ({
              ...item,
              display_name:
                item.product_name || item.product_variant_name || "غير محدد",
              purchase_order_items_quantity:
                item.purchase_order_items_quantity_ordered,
              purchase_order_items_unit_price:
                item.purchase_order_items_unit_cost,
          ...
          order_discount:
            detailedOrder.purchase_orders_order_discount ||
            detailedOrder.purchase_order_order_discount ||
            0,
        };

        // Get supplier and warehouse names
        const supplier = suppliers?.find(
          (s) => s.supplier_id == transformedOrder.purchase_order_supplier_id,
        );
        const warehouse = warehouses?.find(
          (w) => w.warehouse_id == transformedOrder.purchase_order_warehouse_id,
        );
        ...
        const formattedDate = new Date(
          transformedOrder.purchase_order_date,
        ...
        const itemsForPrint = transformedOrder.items.map((item, index) => {
          ...
          const unitPrice = parseFloat(
            item.purchase_order_items_unit_price || 0,
          );
        ...
        const orderLevelDiscount =
          parseFloat(transformedOrder.order_discount || 0) || 0;
  ...
  const handleAdd = async (newData) => {
    ...
    try {
      ...
      setGlobalMessage({
        type: "error",
        message: e.message || "فشل في إضافة أمر الشراء.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdate = async (updatedData) => {
    setLoading(true);
    try {
      const message = await updatePurchaseOrder(
        updatedData.purchase_orders_id,
        updatedData,
      );
      setGlobalMessage({
        type: "success",
        message: message || "تم تحديث أمر الشراء بنجاح!",
      });
      setCurrentView("list");
      await loadAllPurchaseOrderData(true);
    } catch (e) {
      setGlobalMessage({
        type: "error",
        message: e.message || "فشل في تحديث أمر الشراء.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const filteredOrders = useMemo(() => {
    let currentFiltered = Array.isArray(purchaseOrders)
      ? [...purchaseOrders]
      : [];
    ...
    if (selectedSupplierFilter) {
      currentFiltered = currentFiltered.filter((order) => {
        const orderSupplierId =
          order.purchase_orders_supplier_id ??
          order.supplier_id ??
    ...
    if (term) {
      currentFiltered = currentFiltered.filter(
        (order) =>
          ...
          (order.purchase_orders_status &&
            String(order.purchase_orders_status)
              .toLowerCase()
              .includes(term)) ||
          (order.purchase_orders_notes &&
            String(order.purchase_orders_notes).toLowerCase().includes(term)) ||
          // also match supplier id or order id
          (order.purchase_orders_id &&
            String(order.purchase_orders_id).toLowerCase().includes(term)) ||
          (order.purchase_orders_supplier_id &&
            String(order.purchase_orders_supplier_id)
              .toLowerCase()
              .includes(term)),
      );
    }
  ...
  const activeChips = useMemo(() => {
    ...
    if (selectedStatusFilter) {
      chips.push({
        ...
        onRemove: async () => {
          ...
          setCurrentPage(1);
          try {
            await loadAllPurchaseOrderData(true, { status: "", page: 1 });
          } catch {
            /* ignore */
          }
        },
    ...
    if (selectedSupplierFilter) {
      ...
      const found = uniqueSuppliersForFilter.find(
        (opt) => String(opt.value) === String(selectedSupplierFilter),
      );
      ...
      chips.push({
        ...
        onRemove: async () => {
          ...
          setCurrentPage(1);
          try {
            await loadAllPurchaseOrderData(true, { supplier_id: "", page: 1 });
          } catch {
            /* ignore */
          }
        },
    ...
    if (searchTerm && searchTerm.trim() !== "") {
      chips.push({
        ...
        onRemove: async () => {
          ...
          setCurrentPage(1);
          try {
            await loadAllPurchaseOrderData(true, { search: "", page: 1 });
          } catch {
            /* ignore */
          }
        },
        ...
        return (
          <>
            <CustomPageHeader
              title="أوامر الشراء"
              icon={
                <ClipboardDocumentListIcon className="h-6 w-6 text-[#1F2937]" />
              }
            ...
            <FilterBar
              ...
              selectFilters={[
                ...
                },
                {
                  ...
                  options: uniqueSuppliersForFilter,
                },
              ]}
              ...
              <PaginationHeaderFooter
                ...
                totalPages={
                  ...
                  Math.max(
                    1,
                    Math.ceil(
                      (pagination?.total_items ?? filteredOrders.length) /
                        itemsPerPage,
                    ),
                  )
                }
