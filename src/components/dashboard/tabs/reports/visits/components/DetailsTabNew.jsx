  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <div
        ...
        className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm cursor-pointer bg-white hover:border-gray-400 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500"
      >
        <div className="flex items-center justify-between">
          <span
            className={`text-sm ${!dateFrom && !dateTo ? "text-gray-500" : "text-gray-900"}`}
          >
            {label}
          </span>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                className="flex-1 px-3 py-2 bg-indigo-600 text-white text-sm rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
                تطبيق
              </button>
              <button
                ...
                className="flex-1 px-3 py-2 bg-gray-200 text-gray-700 text-sm rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500"
              >
                مسح
              </button>
            </div>
      ...
      columns={[
        ...
        },
        {
          ...
          render: (item) => {
            ...
            const cfg =
              statusConfig[item.visits_status] || statusConfig["Started"];
            const Icon = cfg.icon;
            return (
              <span
                className={`px-2 py-1 rounded-full text-xs font-semibold inline-flex items-center gap-1 ${cfg.color}`}
              >
        ...
        },
        {
          ...
          render: (item) => (
            <div className="flex items-center">
              <BuildingOfficeIcon className="h-4 w-4 text-gray-400 ml-2" />
              <div>
                <div className="text-gray-900 font-medium">
                  {item.clients_company_name || "غير محدد"}
                </div>
                {item.client_area_tag_name && (
        ...
        },
        {
          ...
          render: (item) => (
            <div className="flex items-center">
              ...
              {item.rep_name || "غير محدد"}
            </div>
          ),
        },
        {
          key: "times",
          title: "وقت البداية والنهاية",
          align: "right",
          render: (item) => (
            <div className="flex flex-col">
              <span>البداية: {formatDate(item.visits_start_time)}</span>
              <span className="mt-0.5 text-gray-500">
                النهاية: {formatDate(item.visits_end_time)}
              </span>
            </div>
          ),
        },
        {
          ...
          render: (item) => (
            <div className="flex items-center">
              ...
              {formatDuration(item.visit_duration_minutes)}
            </div>
          ),
        },
        {
            ...
            <div className="flex space-x-4 space-x-reverse text-sm">
              <span className="flex items-center">
                ...
                {(() => {
                  ...
                  const s = item.summary_stats;
                  const a = item.activities;
                  return (
                    s?.total_activities ??
                    (Array.isArray(a) ? a.length : undefined) ??
              ...
              <span className="flex items-center">
                ...
                {(() => {
                  const cached = summaryTotalsByVisit[item.visits_id];
                  if (cached) return cached.total_sales_orders;
                  const s = item.summary_stats;
                  const list = item.orders || item.sales_orders;
                  return (
              ...
              <span className="flex items-center">
                ...
                {(() => {
                  const cached = summaryTotalsByVisit[item.visits_id];
                  if (cached) return cached.total_payments;
                  const s = item.summary_stats;
              ...
              <span className="flex items-center">
                <ArrowUturnLeftIcon className="h-4 w-4 text-red-500 ml-1" />
                {(() => {
                  const cached = summaryTotalsByVisit[item.visits_id];
                  if (cached) return cached.total_returns;
                  const s = item.summary_stats;
                  const list = item.returns || item.sales_returns;
                  return (
                    s?.total_returns ??
                    (Array.isArray(list) ? list.length : undefined) ??
                    item.returns_count ??
...
const DetailsTab = ({ data }) => {
  ...
  const fetchDetails = useCallback(async () => {
    ...
    try {
      ...
      const totalCount =
        result?.pagination?.total ?? (Array.isArray(items) ? items.length : 0);
      const tp =
    ...
    (async () => {
      try {
        ...
        if (!Array.isArray(tags) || tags.length === 0) {
          try {
            tags = await getAppClientAreaTags(false);
          } catch {
            tags = [];
          }
        }
  ...
  const repOptions = useMemo(
    () =>
      representatives.map((r) => ({
        value: String(r.users_id || r.id),
        label: r.users_name || r.name || r.username,
      })),
    [representatives],
  );
  const areaTagOptions = useMemo(
    () =>
      areaTags.map((t) => ({
        value: String(t.client_area_tag_id || t.client_area_tags_id || t.id),
        label: t.client_area_tag_name || t.client_area_tags_name || t.name,
      })),
    [areaTags],
  );
  const clientOptions = useMemo(
    () =>
      (clients || [])
        ...
        .map((c) => ({
          value: String(c.clients_id || c.client_id || c.id),
          label:
            c.clients_company_name || c.company_name || c.client_name || c.name,
        })),
    [clients],
  );
    ...
    try {
      return list.reduce(
        (sum, r) =>
          sum +
          (parseFloat(
            r.returns_total_amount || r.sales_returns_total_amount || 0,
          ) || 0),
        0,
      );
    } catch {
  ...
  const activeChips = useMemo(() => {
    ...
    if (appliedFilters.statusFilter && appliedFilters.statusFilter !== "all") {
      const statusLabel =
        appliedFilters.statusFilter === "Started"
          ? "جارية"
          : appliedFilters.statusFilter === "Completed"
            ? "مكتملة"
            : appliedFilters.statusFilter === "Cancelled"
              ? "ملغاة"
    ...
    if (appliedFilters.repId) {
      const label =
        repOptions.find((o) => String(o.value) === String(appliedFilters.repId))
          ?.label || appliedFilters.repId;
      chips.push({
        key: "rep",
        label: "المندوب",
        value: label,
    ...
    if (appliedFilters.areaTagId) {
      const label =
        areaTagOptions.find(
          (o) => String(o.value) === String(appliedFilters.areaTagId),
        )?.label || appliedFilters.areaTagId;
      chips.push({
        key: "area",
        label: "المنطقة",
        value: label,
        tone: "teal",
        onRemove: () => setAreaTagId(""),
      });
    }
    ...
    if (appliedFilters.clientId) {
      const label =
        clientOptions.find(
          (o) => String(o.value) === String(appliedFilters.clientId),
        )?.label || appliedFilters.clientId;
      chips.push({
        key: "client",
        label: "العميل",
        value: label,
        tone: "purple",
        onRemove: () => setClientId(""),
      });
    }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <FilterBar
        ...
        searchConfig={{
          ...
          onSubmit: (val) => {
            ...
            setAppliedFilters((prev) => ({ ...prev, searchTerm: val }));
            setPage(1);
          },
          onClear: () => {
            setSearchTerm("");
      ...
      {selectedVisit && (
        <div className="fixed inset-0 backdrop-blur-sm bg-black/40 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-2 sm:p-4">
          <div className="relative w-full max-w-6xl bg-white rounded-xl shadow-xl border border-gray-200 max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
            {/* Header */}
            <div className="px-4 py-3 sm:px-6 sm:py-4 border-b border-gray-200 bg-white">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <MapPinIcon className="h-5 w-5 sm:h-6 sm:w-6 text-gray-700" />
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-gray-900">
                    {selectedVisit.clients_company_name}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-gray-900">
                    {getVisitDuration(
                      selectedVisit.visits_start_time,
                      selectedVisit.visits_end_time,
                    )}
                  </span>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-900">
                        {formatDate(selectedVisit.visits_start_time)}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-gray-500">النهاية:</span>
                      <span className="text-gray-900">
                        {formatDate(selectedVisit.visits_end_time)}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="rounded-lg border border-gray-200 p-3 bg-gray-50">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <a
                        href={mapLink(
                          selectedVisit.visits_start_latitude,
                          selectedVisit.visits_start_longitude,
                        )}
                        target="_blank"
                        rel="noreferrer"
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <a
                        href={mapLink(
                          selectedVisit.visits_end_latitude,
                          selectedVisit.visits_end_longitude,
                        )}
                        target="_blank"
                        rel="noreferrer"
                        className="text-blue-700 hover:underline"
            ...
            {/* Body */}
            <div className="overflow-y-auto max-h-[calc(95vh-160px)] sm:max-h-[calc(95vh-200px)] p-3 sm:p-6">
                ...
                <div className="space-y-6">
                          ...
                          <div>
                            ...
                            <div className="text-gray-900">
                              ...
                              <span className="font-semibold">
                                {selectedVisitSummary?.summary_stats
                                  ?.total_sales_orders ??
                                  (selectedVisitSummary?.sales_orders?.length ||
                                    0)}
                              </span>
                    ...
                    <div className="divide-y divide-gray-200">
                      {selectedVisitSummary?.sales_orders?.length > 0 ? (
                        selectedVisitSummary.sales_orders.map((so) => (
                          <div
                            key={so.sales_orders_id}
                            className="p-4 flex items-start justify-between"
                          >
                            <div>
                              <div className="text-sm font-semibold text-gray-900">
                                طلب #{so.sales_orders_id}
                              </div>
                              <div className="text-sm text-gray-700 mt-1">
                                الإجمالي:{" "}
                                <span className="font-bold">
                                  {parseFloat(
                                    so.sales_orders_total_amount || 0,
                                  ).toLocaleString("en-US")}{" "}
                                  {symbol}
                                </span>
                              </div>
                              <div className="text-xs text-gray-500 mt-1">
                                تاريخ الإنشاء:{" "}
                                {new Date(
                                  so.sales_orders_created_at,
                                ).toLocaleString("en-US")}
                              </div>
                            </div>
                            <span className="ml-4 px-2 py-1 rounded border border-gray-300 text-xs text-gray-700">
                              {so.sales_orders_status || "Draft"}
                            </span>
                          </div>
                        ))
                      ) : (
                  ...
                  <div className="rounded-lg border border-gray-200 bg-white">
                    <div className="px-4 py-3 border-b border-gray-200 flex items-center gap-2 text-gray-800">
                      ...
                      <h3 className="font-semibold">
                        المدفوعات ({selectedVisitSummary?.payments?.length || 0}
                        )
                      </h3>
                    </div>
                    <div className="divide-y divide-gray-200">
                        ...
                        selectedVisitSummary.payments.map((p) => (
                          ...
                          >
                            <div>
                              ...
                              <div className="text-xs text-gray-500 mt-1">
                                التاريخ:{" "}
                                {new Date(p.payments_date).toLocaleString(
                                  "en-US",
                                )}
                              </div>
                            </div>
                    ...
                    <div className="divide-y divide-gray-200">
                      {selectedVisitSummary?.documents?.length > 0 ? (
                        selectedVisitSummary.documents.map((d) => (
                          <div
                            key={d.client_document_id || d.client_documents_id}
                            className="p-4 flex items-start justify-between"
                          >
                            <div className="pr-1">
                              <div className="text-sm font-medium text-gray-900">
                                {d.document_type_name || "مستند"}
                              </div>
                              <div className="text-sm text-gray-700 mt-1">
                                {d.client_document_title || "بدون عنوان"}
                              </div>
                              <div className="text-xs text-gray-500 mt-1">
                  ...
                  <div className="rounded-lg border border-gray-200 bg-white">
                    <div className="px-4 py-3 border-b border-gray-200 flex items-center gap-2 text-gray-800">
                      ...
                      <h3 className="font-semibold">
                        المرتجعات ({selectedVisitSummary?.returns?.length || 0})
                      </h3>
                    </div>
                    <div className="divide-y divide-gray-200">
                      {selectedVisitSummary?.returns?.length > 0 ? (
                              ...
                              <div className="text-xs text-gray-500 mt-1">
                                ...
                                {new Date(
                                  r.returns_created_at ||
                                    r.sales_returns_created_at,
                                ).toLocaleString("en-US")}
                              </div>
                            </div>
                            <span className="ml-4 px-2 py-1 rounded border border-gray-300 text-xs text-gray-700">
                              {r.returns_status || "Draft"}
                            </span>
                          </div>
                        ))
                      ) : (
                  ...
                  <div className="rounded-lg border border-gray-200 bg-white">
                    <div className="px-4 py-3 border-b border-gray-200 flex items-center gap-2 text-gray-800">
                      ...
                      <h3 className="font-semibold">
                        الأنشطة ({selectedVisitSummary?.activities?.length || 0}
                        )
                      </h3>
                    </div>
                    <div className="divide-y divide-gray-200">
                      {selectedVisitSummary?.activities?.length > 0 ? (
                        selectedVisitSummary.activities.map((a) => (
                          ...
                          >
                            <div>
                              <div className="text-sm font-medium text-gray-900">
                                {getActivityTypeName(a.activity_type)}
                              </div>
                            ...
                            <div className="text-left text-xs text-gray-500 whitespace-nowrap ml-4">
                              {new Date(a.activity_timestamp).toLocaleString(
                                "en-US",
                              )}
                            </div>
                          </div>
            ...
            <div className="px-6 py-3 border-t border-gray-200 bg-white flex justify-between items-center text-sm text-gray-600">
              <div>
                آخر تحديث:{" "}
                {new Date(selectedVisit.visits_updated_at).toLocaleString(
                  "en-US",
                )}
              </div>
              <button
                onClick={() => setSelectedVisit(null)}
                className="px-4 py-1.5 rounded border border-gray-300 hover:bg-gray-50 text-gray-800"
              >
                إغلاق
              </button>
            </div>
