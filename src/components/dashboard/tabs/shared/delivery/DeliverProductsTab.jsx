...
const OrderItemRow = ({
  ...
  return (
    <tr className="[&>td]:border [&>td]:p-1 hover:bg-gray-50 text-xs text-gray-700">
      ...
      <td className="text-center font-semibold text-gray-900 w-16">
        {formatNumber(totalQuantity)}
      </td>
      <td className="text-center text-gray-500 w-16">
        {formatNumber(deliveredQuantity)}
      </td>
      <td className="text-center text-gray-500 w-16">
        {formatNumber(returnedQuantity)}
      </td>
      <td className="text-center font-semibold text-green-600 w-16">
      ...
      <td className="text-center w-16">
        <NumberInput
          ...
          onChange={(val) =>
            onQuantityChange(
              Math.min(Math.max(0, parseFloat(val) || 0), remainingQuantity),
            )
          }
...
export default function DeliverProductsTab() {
  ...
  const activeChips = useMemo(() => {
    ...
    if (selectedClient) {
      const client = clients.find(
        (c) => c.clients_id.toString() === selectedClient,
      );
      chips.push({
        key: "client",
  ...
  const keepEntriesForOrder = (
    obj,
    orderId,
    { includeOrderLevel = false } = {},
  ) => {
    ...
    return Object.fromEntries(
      Object.entries(obj).filter(
        ([key]) =>
          key.startsWith(prefix) || (includeOrderLevel && key === orderKey),
      ),
    );
  };
  ...
  const handleItemToggle = async (orderId, itemId, item) => {
    ...
    if (!isCurrentlySelected) {
      const baseSelected = switchingOrder
        ? {}
        : keepEntriesForOrder(selectedItems, orderId);
      baseSelected[key] = true;
      setSelectedItems(baseSelected);

      const baseDetails = switchingOrder
        ? {}
        : keepEntriesForOrder(deliveryDetails, orderId, {
            includeOrderLevel: true,
          });
      if (item) {
        const deliveredQuantity = parseFloat(
      ...
      const baseAvailableBatches = switchingOrder
        ? {}
        : keepEntriesForOrder(availableBatches, orderId);
      const baseSelectedBatches = switchingOrder
        ? {}
        : keepEntriesForOrder(selectedBatches, orderId);
      ...
      const remainingForOrder = Object.keys(updatedSelected).some((k) =>
        k.startsWith(`${orderId}-`),
      );
    ...
    if (shouldSelect) {
      ...
      deliverableItems.forEach((item) => {
        ...
        if (remainingQuantity > 0)
          newDetails[key] = { quantity: remainingQuantity };
      });
      // Keep order-level details if present
      const orderNotes = deliveryDetails[order.sales_orders_id]
        ? { [order.sales_orders_id]: deliveryDetails[order.sales_orders_id] }
        : {};
  ...
  const handlePrintDeliveryReceipt = (order) => {
    if (activeOrderId !== order.sales_orders_id) {
      setGlobalMessage({
        type: "error",
        message: "يرجى اختيار عناصر من هذا الطلب أولاً",
      });
      return;
    }
    ...
    if (selectedOrderItems.length === 0) {
      setGlobalMessage({
        type: "error",
        message: "يرجى اختيار عنصر واحد على الأقل للطباعة",
      });
      return;
    }
  ...
  const handleDeliveryClick = (orderId) => {
    ...
    if (activeOrderId !== orderId) {
      setGlobalMessage({
        type: "error",
        message: "يرجى تحديد عناصر من هذا الطلب أولًا",
      });
      return;
    }
    ...
    if (selectedOrderItems.length === 0) {
      setGlobalMessage({
        type: "error",
        message: "يرجى اختيار عنصر واحد على الأقل للتسليم",
      });
      return;
    }
  ...
  const handleSubmitDelivery = async (orderId) => {
    try {
      ...
      setGlobalMessage({
        type: "error",
        message: err.message || "فشل في إنشاء التسليم",
      });
    } finally {
      ...
      <div className="space-y-4">
        {paginatedOrders.length === 0 ? (
          ...
          paginatedOrders.map((order) => {
            ...
            const deliveryData = isActiveOrder
              ? deliveryDetails[order.sales_orders_id] || {}
              : {};
              ...
              >
                <div className="flex flex-wrap gap-4 text-xs text-gray-700">
                  <span className="font-semibold text-gray-900">
                    طلب #{order.sales_orders_id}
                  </span>
                  <span>
                    التاريخ:{" "}
                    {order.sales_orders_order_date
                      ? format(
                          new Date(order.sales_orders_order_date),
                          "dd/MM/yyyy HH:mm",
                        )
                      : "-"}
                  </span>
                    ...
                    <div className="ms-auto flex flex-wrap items-center gap-2">
                      ...
                      {selectedCount > 0 && (
                        <button
                          type="button"
                          onClick={() =>
                            handleToggleOrderSelection(order, false)
                          }
                  ...
                  <table className="w-full border border-gray-300 text-xs">
                    <thead className="bg-gray-50">
                      <tr className="[&>th]:border [&>th]:p-2 text-gray-600">
                        ...
                        <th className="text-right w-40 min-w-0 max-w-[220px] truncate">
                          المنتج
                        </th>
                      ...
                      {deliverableItems.map((item) => {
                        ...
                        return (
                          <OrderItemRow
                            ...
                            selectedBatch={
                              isActiveOrder ? selectedBatches[key] || "" : ""
                            }
                            availableBatches={
                              isActiveOrder ? availableBatches[key] || [] : []
                            }
                          />
                        );
                      })}
                ...
                {hasSelectedItems && (
                  <div className="border rounded p-3 bg-gray-50 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2 text-xs">
                      ...
                      <button
                        onClick={() =>
                          handleDeliveryClick(order.sales_orders_id)
                        }
                        disabled={isSubmitting}
                        className="px-4 py-1 rounded bg-[#8DD8F5] text-black disabled:opacity-50 inline-flex items-center gap-2"
                      >
                        <TruckIcon className="h-4 w-4" />
                        {isSubmitting
                          ? "جاري التسليم..."
                          : "تسليم المنتجات المحددة"}
                      </button>
      ...
      {confirmDialog.isOpen &&
        ...
        (() => {
          const order = orders.find(
            (o) => o.sales_orders_id === confirmDialog.orderId,
          );
              ...
              <div className="bg-white rounded-lg shadow-lg w-11/12 md:w-2/3 lg:w-1/2 p-3 sm:p-4 max-h-[95vh] overflow-y-auto">
                    ...
                    <tbody>
                      {selectedOrderItems.map((it, idx) => {
                        ...
                        const qty = parseFloat(
                          deliveryDetails[key]?.quantity || 0,
                        );
                        const batch =
                          selectedBatches[key] ||
                          it.batch_date ||
                          it.inventory_production_date ||
                          "";
                        const batchDisplay = batch
                          ? new Date(batch).toLocaleDateString("ar-EG")
                          : "-";
                        const packaging =
                        ...
                        const name = it.variant_name
                          ...
                          : it.products_name || it.variant_name || "غير محدد";
                        return (
                          <tr
                            key={it.sales_order_items_id}
                            className="border-t"
                          >
                ...
                <div className="flex justify-end gap-3">
                  ...
                  <button
                    onClick={async () => {
                      setConfirmDialog({ isOpen: false, orderId: null });
                      await handleSubmitDelivery(order.sales_orders_id);
                    }}
