...
export default function TransfersTab() {
  ...
  const [isTransferDetailsModalOpen, setIsTransferDetailsModalOpen] =
    useState(false);
  ...
  const loadAllTransferData = useCallback(
    async (forceApiRefresh = false) => {
      if (forceApiRefresh) {
        setLoading(true);
        setGlobalMessage({
          type: "info",
          message: "جاري تحديث بيانات التحويلات...",
        });
      }
      setError(null);
      try {
        const [
          ...
          getTransfersPaginated({
            ...
            destination_warehouse_id:
              selectedDestinationWarehouseFilter || undefined,
          }),
        ...
        const extractedTransfers = Array.isArray(transfersResp)
          ? transfersResp
          : transfersResp?.data || [];
        ...
        setGlobalMessage({
          type: "error",
          message: `فشل في تحميل بيانات التحويلات: ${errorMessage}`,
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleEnterAddView = useCallback(async () => {
    ...
    try {
      ...
      setGlobalMessage({
        type: "error",
        message: e.message || "فشل في تجهيز نموذج التحويل.",
      });
  ...
  const refreshSupportingDataForAdd = useCallback(async () => {
    ...
    try {
      ...
      setGlobalMessage({
        type: "error",
        message: e.message || "فشل في تحديث بيانات النموذج.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const enrichedInventoryForForm = useMemo(() => {
    ...
    return allInventoryItems.map((item) => {
      ...
      return {
        ...
        packaging_type_name:
          packagingType?.packaging_types_name || "Unknown Packaging",
        base_unit_name: baseUnit?.base_units_name || "Unknown Unit",
        packaging_types_default_conversion_factor:
          packagingType?.packaging_types_default_conversion_factor || 1,
      };
    });
  ...
  const handleAdd = async (newData) => {
    ...
    try {
      ...
      setGlobalMessage({
        type: "success",
        message: message || "تم إضافة التحويل بنجاح!",
      });
      setCurrentView("list");
      ...
      setGlobalMessage({
        type: "error",
        message: e.message || "فشل في إضافة التحويل.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmStatusUpdate = async () => {
    ...
    setLoading(true);
    try {
      const message = await updateTransferStatus(
        transferToUpdate.transfer_id,
        newStatusForUpdate,
      );
      ...
      setGlobalMessage({
        type: "success",
        message:
          message || `تم تحديث حالة التحويل إلى ${newStatusForUpdate} بنجاح!`,
      });
      ...
      setGlobalMessage({
        type: "error",
        message: e.message || "فشل في تحديث حالة التحويل.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const openTransferDetailsModal = async (transferSummary) => {
    if (transferSummary?.type === "request") {
      ...
      const full = (requests || []).find(
        (r) => r.request_id === transferSummary.transfer_id,
      );
      if (!full) return;
    ...
    setError(null);
    try {
      const detailedTransfer = await getTransferDetails(
        transferSummary.transfer_id,
      );
      ...
      setError(
        "فشل في تحميل تفاصيل التحويل: " + (err.message || "خطأ غير معروف"),
      );
      setGlobalMessage({
        type: "error",
        message: "فشل في تحميل تفاصيل التحويل.",
      });
    } finally {
  ...
  const handleApproveAllocateRequest = async (
    ...
    try {
      ...
      setGlobalMessage({
        type: "error",
        message: e.message || "فشل في الموافقة على الطلب.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const activeChips = useMemo(() => {
    ...
    if (selectedStatusFilter) {
      const option = transferStatusOptions.find(
  ...
  const filteredTransfers = useMemo(() => {
      ...
      currentFiltered = currentFiltered.filter(
        (transfer) =>
          ...
          warehouses
            .find(
              (w) =>
                w.warehouse_id === transfer.transfer_destination_warehouse_id,
            )
            ?.warehouse_name.toLowerCase()
            .includes(term),
      );
    ...
    if (selectedStatusFilter) {
      currentFiltered = currentFiltered.filter(
        (transfer) => transfer.status === selectedStatusFilter,
      );
    }
    if (selectedSourceWarehouseFilter) {
      currentFiltered = currentFiltered.filter(
        (transfer) =>
          String(transfer.transfer_source_warehouse_id) ===
          selectedSourceWarehouseFilter,
      );
    }
    if (selectedDestinationWarehouseFilter) {
      currentFiltered = currentFiltered.filter(
        (transfer) =>
          String(transfer.transfer_destination_warehouse_id) ===
          selectedDestinationWarehouseFilter,
      );
    }
          ...
          return (
            <div className="text-center text-gray-600 mt-8">
              ...
              {warehouses.length === 0 && (
                <Alert
                  type="warning"
                  message="لا توجد مخازن متاحة. يرجى إضافة مخازن أولاً."
                  className="mt-4"
                />
              )}
              {products.length === 0 && (
                <Alert
                  type="warning"
                  message="لا توجد منتجات متاحة. يرجى إضافة منتجات أولاً."
                  className="mt-4"
                />
              )}
              {packagingTypes.length === 0 && (
                <Alert
                  type="warning"
                  message="لا توجد أنواع تعبئة متاحة. يرجى إضافة أنواع تعبئة أولاً."
                  className="mt-4"
                />
              )}
              {baseUnits.length === 0 && (
                <Alert
                  type="warning"
                  message="لا توجد وحدات قياس أساسية متاحة. يرجى إضافة وحدات أولاً."
                  className="mt-4"
                />
              )}
            </div>
        ...
        return (
          <>
            <CustomPageHeader
              ...
              icon={<ArrowPathIcon className="h-8 w-8 text-[#1F2937]" />}
            ...
            {!loading && (
              <PaginationHeaderFooter
                total={Number(
                  serverPagination?.total || filteredTransfers.length,
                )}
              ...
              <GlobalTable
                ...
                renderRow={(transfer) => {
                  const sourceWarehouse = warehouses.find(
                    (w) =>
                      w.warehouse_id === transfer.transfer_source_warehouse_id,
                  );
                  ...
                  return (
                    <>
                      <td className="px-4 py-4 text-sm font-medium text-[#1F2937] ">
                        {displayId}
                      </td>
                      ...
                      <td className="px-4 py-4 text-sm text-gray-500 ">
                        {destinationWarehouse?.warehouse_name ||
