        ...
        <div className="overflow-x-auto">
          <GlobalTable
            ...
            renderRow={(r) => {
              ...
              return (
                <>
                  <td className="px-6 py-4 text-sm font-medium text-blue-600 ">#{r.receipt_id}</td>
                  {odooEnabled && (
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500 "><div className="flex items-center"><CalendarIcon className="h-4 w-4 ml-1 text-gray-400" />{dt.full}</div></td>
                  <td className="px-6 py-4 text-sm text-gray-500  max-w-[180px] truncate" title={r.notes}>{r.notes || '-'}</td>
...
transition-all duration-200 hover:scale-110" onClick={() => handlePrintReceipt(r)} title="طباعة"><PrinterIcon className="h-4 w-4" /></button>
                      <button                             className="p-1.5 rounded-full 
                   text-sky-700 bg-sky-100
                   hover:bg-sky-500 hover:text-white
                   hover:shadow-[0_0_12px_rgba(56,189,248,0.45)]
                   transition-all duration-200 hover:scale-110" onClick={() => handleViewDetails(r)} title="عرض"><EyeIcon className="h-4 w-4" /></button>
                    </div>
                  </td>
                </>
              );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="relative w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 max-h-[85vh] overflow-y-auto rounded-2xl bg-white shadow-2xl border border-gray-100">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b bg-gradient-to-r from-[#8DD8F520] to-transparent rounded-t-2xl">
              ...
              <button
                onClick={() => { setShowDetailModal(false); setSelectedReceipt(null); }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6">
              ...
              {[
                ...
                ["رقم الطلب المرتبط", getPurchaseOrderId(selectedReceipt) ? `#${getPurchaseOrderId(selectedReceipt)}` : "غير محدد"],
                ["المخزن", selectedReceipt.warehouse_name || "غير محدد"],
                ["المستلم", selectedReceipt.received_by_user_name || "غير محدد"],
              ].map(([label, value], i) => (
              ...
              <div className="space-y-1">
                <p className="text-xs font-medium text-gray-500">تاريخ الاستلام</p>
                <div className="bg-gray-50 rounded-xl px-3 py-2 shadow-inner">
                  <p className="font-medium text-[#1F2937]">
                    {formatDateTime(selectedReceipt.receipt_date || selectedReceipt.created_at).full}
                  </p>
                  <div className="flex items-center text-xs text-gray-500 mt-1">
                    <CalendarIcon className="h-4 w-4 ml-1" />
                    {formatDateTime(selectedReceipt.receipt_date || selectedReceipt.created_at).date}
                    <span className="mx-2">•</span>
                    {formatDateTime(selectedReceipt.receipt_date || selectedReceipt.created_at).time}
                  </div>
                </div>
              </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {selectedReceipt.items.map((item, index) => {
                        const prod = item.production_date || item.goods_receipt_items_production_date || '';
                        const prodDisplay = prod ? new Date(prod).toLocaleDateString('ar-EG') : '-';
                        const packaging = item.packaging_type_name || item.packaging_name || '-';

                        return (
                          <tr key={index} className="border-t hover:bg-[#f7fdff] transition">
                            <td className="py-3 text-center text-gray-500">{index + 1}</td>
                            <td className="py-3 font-medium text-[#1F2937]">{item.variant_name || item.product_name || 'غير محدد'}</td>
                            <td className="py-3 text-center text-gray-500">{item.variant_sku || '-'}</td>
                            <td className="py-3 text-center text-gray-500">{packaging}</td>
                            <td className="py-3 text-center font-semibold text-emerald-600">
                              {parseFloat(item.quantity_received || 0).toFixed(2)}
                            </td>
                            <td className="py-3 text-center text-gray-700">{prodDisplay}</td>
                          </tr>
                        );
                      })}
                    </tbody>
