...
function WarehousesTab() {
  ...
  const [isWarehouseDetailsModalOpen, setIsWarehouseDetailsModalOpen] =
    useState(false);
  ...
  const [selectedContactPersonFilter, setSelectedContactPersonFilter] =
    useState("");
  ...
  const handleAddWarehouse = async (newWarehouseData) => {
    try {
      ...
      const successMsg =
        typeof result === "object" && result.warehouse_code
      ...
      setGlobalMessage({
        type: "error",
        message: err.message || "فشل في إضافة المخزن.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdateWarehouse = async (updatedWarehouseData) => {
    try {
      ...
      const message = await updateWarehouse(
        updatedWarehouseData.warehouse_id,
        updatedWarehouseData,
      );
      setGlobalMessage({
        type: "success",
        message: message || "تم تحديث المخزن بنجاح!",
      });
      setCurrentView("list");
      ...
      setGlobalMessage({
        type: "error",
        message: err.message || "فشل في تحديث المخزن.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteWarehouse = async () => {
    ...
    try {
      ...
      setGlobalMessage({
        type: "success",
        message: message || "تم حذف المخزن بنجاح!",
      });
      setCurrentView("list");
      ...
      setGlobalMessage({
        type: "error",
        message: err.message || "فشل في حذف المخزن.",
      });
    } finally {
  ...
  const contactPersonOptions = useMemo(() => {
    const options = [
      ...
      ...uniqueContactPersons.map((person) => ({
  ...
  const activeChips = useMemo(() => {
    ...
    if (selectedContactPersonFilter) {
      const option = contactPersonOptions.find(
        (o) => o.value === selectedContactPersonFilter,
      );
      if (option)
        chips.push({
          key: "contactPerson",
          label: "الشخص المسؤول",
          value: option.label,
          onRemove: () => setSelectedContactPersonFilter(""),
        });
    }
    return chips;
  }, [
    ...
    if (selectedTypeFilter && selectedTypeFilter.trim()) {
      currentFiltered = currentFiltered.filter(
        (w) =>
          ...
          w.warehouse_type.trim().toLowerCase() ===
            selectedTypeFilter.trim().toLowerCase(),
      );
    }
    if (selectedStatusFilter && selectedStatusFilter.trim()) {
      currentFiltered = currentFiltered.filter(
        (w) =>
          ...
          w.warehouse_status.trim().toLowerCase() ===
    ...
    if (selectedContactPersonFilter && selectedContactPersonFilter.trim()) {
      currentFiltered = currentFiltered.filter(
        (w) =>
          ...
          String(w.warehouse_representative_user_id) ===
            String(selectedContactPersonFilter.trim()),
      );
    }
        ...
        return (
          <>
            <CustomPageHeader
              ...
              icon={
                <BuildingStorefrontIcon className="h-8 w-8 text-[#1F2937]" />
              }
                  ...
                  return (
                    <>
                      ...
                      <td className="px-4 py-3 text-gray-600 ">
                        {getUserName
                          ? getUserName(
                              warehouse.warehouse_representative_user_id,
                            )
                          : "–"}
                      </td>
