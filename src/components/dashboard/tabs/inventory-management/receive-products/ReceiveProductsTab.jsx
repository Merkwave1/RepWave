...
export default function ReceiveProductsTab() {
    ...
    async (forceRefresh = false) => {
      ...
      try {
        ...
        const sortedOrders = [...allOrders].sort((a, b) => {
          const da = new Date(
            a.purchase_orders_order_date || a.created_at || 0,
          ).getTime();
          const db = new Date(
            b.purchase_orders_order_date || b.created_at || 0,
          ).getTime();
        ...
        const pendingOrdersData = sortedOrders.map((order) => ({
          ...
          items: (order.items || []).map((item) => ({
            ...
            quantity_returned: parseFloat(
              item.purchase_order_items_quantity_returned || 0,
            ),
          })),
        }));
        ...
        setGlobalMessage({
          type: "error",
          message: "فشل في تحميل البيانات اللازمة.",
        });
      } finally {
        setLoading(false);
      }
      ...
      for (const product of products) {
        ...
        if (variant) {
          return variant.variant_name
            ? `${product.products_name} - ${variant.variant_name}`
  ...
  const filteredAndSortedOrders = useMemo(() => {
    ...
    const warehouseFilter =
      selectedWarehouse && typeof selectedWarehouse === "object"
    ...
    if (warehouseFilter) {
      ordersArray = ordersArray.filter(
        (order) =>
          String(order.purchase_orders_warehouse_id) ===
    ...
    if (supplierFilter) {
      ordersArray = ordersArray.filter(
        (order) =>
          String(order.purchase_orders_supplier_id) === String(supplierFilter),
      );
    }
    ...
    if (dateFrom) {
      ...
      ordersArray = ordersArray.filter(
        (order) => new Date(order.purchase_orders_order_date) >= from,
      );
    }
    ...
    if (dateTo) {
      ...
      ordersArray = ordersArray.filter(
        (order) => new Date(order.purchase_orders_order_date) <= to,
      );
    }
  ...
  const getProductionDateForItem = useCallback(
    (orderId, itemId) => {
      const key = makeItemKey(orderId, itemId);
  ...
  const getProductUnitById = useCallback(
    (variantId) => {
  ...
  const printReceiptDocument = useCallback(
    (order) => {
      if (
        activeOrderId !== null &&
        activeOrderId !== order.purchase_orders_id
      ) {
        setGlobalMessage({
          type: "warning",
          message:
            "يرجى إكمال أو إلغاء تحديد الأمر الحالي قبل طباعة أمر مختلف.",
        });
        return;
      }
      ...
      const supplier = Array.isArray(suppliers)
        ? suppliers.find(
            (s) => s.supplier_id === order.purchase_orders_supplier_id,
          )
        : null;
      const warehouse = Array.isArray(warehouses)
        ? warehouses.find(
            (w) => w.warehouse_id === order.purchase_orders_warehouse_id,
          )
        : null;

      const itemsToReceive = order.items.filter(
        (item) =>
          getQuantityForItem(
      ...
      if (itemsToReceive.length === 0) {
        setGlobalMessage({
          type: "error",
          message: "يرجى اختيار كميات للاستلام قبل الطباعة",
        });
        return;
      }
          ...
          if (variant) {
            return {
              sku:
                ...
                variantId,
              packaging:
                variant.packaging_type_name ||
                variant.packaging ||
                variant.packaging_type ||
                getProductUnitById(variantId),
            };
          }
        }
        return { sku: variantId, packaging: getProductUnitById(variantId) };
      ...
      const itemsForPrint = itemsToReceive.map((item, index) => {
        ...
        const name =
          item.variant_name ||
          item.products_name ||
          getProductNameById(item.purchase_order_items_variant_id);
        const variantDetails = getVariantDetails(
          item.purchase_order_items_variant_id,
        );
        const quantity = getQuantityForItem(
          order.purchase_orders_id,
          item.purchase_order_items_id,
        );
        const packaging =
          ...
          variantDetails.packaging;
        const productionDate = getProductionDateForItem(
          order.purchase_orders_id,
          item.purchase_order_items_id,
        );
        return {
    ...
    <body>
      ...
      const doPrint = async () => {
        try {
          ...
          const ok = await printHtmlUtil(printHtml, {
            title: "إيصال استلام بضائع",
            closeAfter: 700,
          });
          if (!ok) {
            setGlobalMessage({
              type: "error",
              message: "فشل في طباعة الإيصال. يرجى السماح بالنوافذ المنبثقة.",
            });
          }
        } catch (e) {
  ...
  const handleQuantityChange = (order, item, rawValue) => {
    ...
    if (activeOrderId !== null && activeOrderId !== orderId) {
      setGlobalMessage({
        type: "warning",
        message: "يرجى إكمال أو إلغاء تحديد الأمر الحالي قبل تعديل أمر مختلف.",
      });
      return;
    }
    ...
    if (!Number.isNaN(numericValue) && numericValue > 0) {
      setProductionDates((prev) => {
        ...
        if (!base[itemKey]) {
          base[itemKey] =
            prev[itemKey] || new Date().toISOString().split("T")[0];
        }
        return base;
      });
  ...
  const handleSubmitReceiving = async (orderId) => {
    ...
    const order = pendingOrders.find(
      (order) => order.purchase_orders_id === orderId,
    );
    if (!order) {
    ...
    if (activeOrderId !== null && activeOrderId !== orderId) {
      setGlobalMessage({
        type: "warning",
        message: "يرجى إكمال أو إلغاء تحديد الأمر الحالي قبل استلام أمر مختلف.",
      });
      setIsSubmitting(false);
      return;
    }
    ...
    if (itemsToSubmit.length === 0) {
      setGlobalMessage({
        type: "warning",
        message: "الرجاء إدخال الكميات المراد استلامها.",
      });
      setIsSubmitting(false);
      return;
    }
    ...
    try {
      ...
      setGlobalMessage({
        type: "error",
        message: e.message || "فشل في تسجيل الاستلام.",
      });
    } finally {
      setIsSubmitting(false);
    }
    ...
    <div className="p-4" dir="rtl">
      ...
      <div className="space-y-4">
        ...
        {paginatedOrders.map((order) => {
          const orderId = order.purchase_orders_id;
          const hasSelectedItems = order.items.some(
            (item) =>
              getQuantityForItem(orderId, item.purchase_order_items_id) > 0,
          );
          const selectableItems = order.items.filter(
            (item) => parseFloat(item.quantity_pending || 0) > 0,
          );
          const anySelectableSelected = selectableItems.some(
            (item) =>
              getQuantityForItem(orderId, item.purchase_order_items_id) > 0,
          );
          return (
      ...
      {showConfirmModal && confirmOrder && (
        <div className="fixed inset-0 backdrop-blur-sm bg-black/40 z-50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white rounded-lg shadow-lg w-11/12 md:w-2/3 lg:w-1/2 p-3 sm:p-4 max-h-[95vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-3">
              هل أنت متأكد أنك تريد استلام المنتجات التالية؟
            </h3>
            <p className="text-sm text-gray-600 mb-3">
              (تاريخ الإنتاج الموضح لكل صنف)
            </p>
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="p-2 text-center">تاريخ الانتاج</th>
                  </tr>
                </thead>
                <tbody>
                  {confirmOrder.items
                    .filter(
                      (it) =>
                        getQuantityForItem(
                          confirmOrder.purchase_orders_id,
                          it.purchase_order_items_id,
                        ) > 0,
                    )
                    .map((it, idx) => {
                      const qty = getQuantityForItem(
                        confirmOrder.purchase_orders_id,
                        it.purchase_order_items_id,
                      );
                      const prod =
                        ...
                        it.production_date ||
                        it.goods_receipt_items_production_date ||
                        "";
                      ...
                      const packaging =
                        ...
                        it.base_units_name ||
                        getProductUnitById(it.purchase_order_items_variant_id);
                      const name = it.variant_name
                        ? `${it.products_name} - ${it.variant_name}`
                        : it.products_name ||
                          getProductNameById(
                            it.purchase_order_items_variant_id,
                          );
                      return (
                        <tr
                          key={it.purchase_order_items_id}
                          className="border-t"
                        >
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-1 bg-[#1F2937] text-white rounded"
              >
                نعم
              </button>
            </div>
