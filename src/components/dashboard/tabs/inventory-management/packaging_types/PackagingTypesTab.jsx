...
export default function PackagingTypesTab() {
  ...
  const loadPackagingTypesAndBaseUnits = useCallback(async () => {
    ...
    try {
      const [typesData, unitsData] = await Promise.all([
        mockPackagingTypesApi.getAllPackagingTypes(),
        mockBaseUnitsApi.getAllBaseUnits(),
      ]);
      ...
      setGlobalMessage({
        type: "error",
        message: "Failed to load packaging types data.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleAdd = async (formData) => {
    setLoading(true);
    try {
      await mockPackagingTypesApi.addPackagingType(formData);
      setGlobalMessage({
        type: "success",
        message: "Packaging type added successfully!",
      });
      setCurrentView("list");
      ...
      setGlobalMessage({
        type: "error",
        message: e.message || "Failed to add packaging type.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdate = async (formData) => {
    setLoading(true);
    try {
      await mockPackagingTypesApi.updatePackagingType(
        selectedPackagingType.packaging_types_id,
        formData,
      );
      setGlobalMessage({
        type: "success",
      ...
      setGlobalMessage({
        type: "error",
        message: e.message || "Failed to update packaging type.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      await mockPackagingTypesApi.deletePackagingType(
        selectedPackagingType.packaging_types_id,
      );
      setGlobalMessage({
        type: "success",
      ...
      setGlobalMessage({
        type: "error",
        message: e.message || "Failed to delete packaging type.",
      });
    } finally {
  ...
  const isOverlay = ["add", "edit", "details", "deleteConfirm"].includes(
    currentView,
  );
